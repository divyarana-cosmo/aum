...
def lnprior(x):
    logM0, logM1, gamma1, gamma2, sig0, b0, b1, b2, alpsat, cfac, poff, roff , ap = x
    if 8.0<=logM0<=11.5 and 10.0<=logM1<=15 and 2<=gamma1<=5 and 0.01<=gamma2<=2 and 0.05<=sig0<=0.5 and -2<=b0<=2 and -1<=b1<=2 and -1<=b2<=2 and -2<=alpsat<=-1  and 0.5<=ap<=5  and 0<=poff<=1.0 and 0.0<=roff<=0.2 and cfac>0:
...
if __name__ == "__main__":
    ...
    import sys
    chn = pd.read_csv('./jk_preet_gama_output_cosmo_y08_fits/chainfile.dat_bosch_2013_removed_first_4_bins', delim_whitespace=1, header=None)
    ...
    #logM0, logM1, gamma1, gamma2, sig0, b0, b1, b2, alpsat, cfac, poff, roff , ap = x
    p_logM0    = chn.values[-nwalkers:,0]
    p_logM1    = chn.values[-nwalkers:,1]
    p_gamma1   = chn.values[-nwalkers:,2]
    p_gamma2   = chn.values[-nwalkers:,3]
    p_sig0     = chn.values[-nwalkers:,4]
    p_b0       = chn.values[-nwalkers:,5]
    p_b1       = chn.values[-nwalkers:,6]
    p_b2       = chn.values[-nwalkers:,7]
    p_alpsat   = chn.values[-nwalkers:,8]
    p_cfac     = chn.values[-nwalkers:,9]
    p_poff     = chn.values[-nwalkers:,10]
    p_roff     = chn.values[-nwalkers:,11]
    p_ap       = chn.values[-nwalkers:,12]
    ...
    Nburn=20000
    pos = runchain(Nburn,sampler,"./jk_preet_gama_output_cosmo_y08_fits/rechainfile.dat_bosch_2013_removed_first_4_bins","./jk_preet_gama_output_cosmo_y08_fits/repredfile.dat_bosch_2013_removed_first_4_bins",p_0);
