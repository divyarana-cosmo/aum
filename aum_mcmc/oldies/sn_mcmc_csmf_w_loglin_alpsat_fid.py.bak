...
def lnprior(x):
    ...
    if 8.0<=logM0<=11.5 and 9.5<=logM1<=15 and 1<=gamma1<=10 and 0.01<=gamma2<=2 and 0.05<=sig0<=0.5 and -2<=b0<=2 and -2<=b1<=2 and -2<=b2<=2 and -2<=alpsat<=-1 and -1<=eta<=0.0 and 0.5<=ap<=5  and 0<=poff<=1.0 and 0.0<=roff<=0.2 and cfac>0:
...
if __name__ == "__main__":
    ...
    p_gamma1   = np.random.uniform(1,10,nwalkers)
    ...
    p_poff     = 0.0*np.random.uniform(0.0,1.0,nwalkers)
    p_roff     = 0.1+0.0*np.random.uniform(0.0,0.2,nwalkers)
    ...
    Nburn=5000
    pos = runchain(Nburn,sampler,"./sn_preet_gama_output_cosmo_zu15_full_revised_fits/burnfile_bosch_2013_loglin_alpsat_uni_const_alpha_no_off.dat_wide_gamma1","./sn_preet_gama_output_cosmo_zu15_full_revised_fits/burnpredfile_bosch_2013_loglin_alpsat_uni_const_alpha_no_off.dat_wide_gamma1",p_0);
    ...
    Ntotal=10000
    posfinal = runchain(Ntotal,sampler,"./sn_preet_gama_output_cosmo_zu15_full_revised_fits/chainfile_bosch_2013_loglin_alpsat_uni_const_alpha_no_off.dat_wide_gamma1","./sn_preet_gama_output_cosmo_zu15_full_revised_fits/predfile_bosch_2013_loglin_alpsat_uni_const_alpha_no_off.dat_wide_gamma1", pos);
    #posfinal = runchain(Ntotal,sampler,"/scratch/cdivya/chainfile_bosch_2013_loglin_alpsat_uni_const_alpha_no_off.dat","/scratch/cdivya/predfile_bosch_2013_loglin_alpsat_uni_const_alpha_no_off.dat", pos);
    pool.close()
