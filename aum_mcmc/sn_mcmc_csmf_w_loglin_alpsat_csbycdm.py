...
def esd(x, rbin, zred, logMa, logMb, logMstel, aum, whichopt=0):
    logM0, logM1, gamma1, gamma2, sig0, b0, b1, b2, alpsat, eta, cfac, poff, roff , csbycdm, ap = x
...
def lnprior(x):
    logM0, logM1, gamma1, gamma2, sig0, b0, b1, b2, alpsat, eta, cfac, poff, roff , csbycdm, ap = x
    if 8.0<=logM0<=11.5 and 9.5<=logM1<=15 and 2<=gamma1<=5 and 0.01<=gamma2<=2 and 0.05<=sig0<=0.5 and -2<=b0<=2 and -2<=b1<=2 and -2<=b2<=2 and -2<=alpsat<=-1 and -1<=eta<0 and 0.5<=ap<=5  and 0<=poff<=1.0 and 0.0<=roff<=0.2 and cfac>0 and 0.1<=csbycdm<=3.0:
...
<<<<<<< HEAD
    ...
    print('logM0\tlogM1\tgamma1\tgamma2\tsig0\tb0\tb1\tb2\talpsat\teta\tcfac\tap\tpoff\troff\tcsbycdm\tap\tchisq')
    print((x,chisq))
...
if __name__ == "__main__":
    ...
    p_csbycdm  = np.random.uniform(0.1,3.0,nwalkers)
...
<<<<<<< HEAD
    p_0 = list(zip(p_logM0, p_logM1, p_gamma1, p_gamma2, p_sig0, p_b0, p_b1, p_b2, p_alpsat, p_eta, p_cfac, p_poff, p_roff, p_csbycdm, p_ap))

    
    print(("Starting iteration, ndim is ",ndim));
=======
    p_0 = zip(p_logM0, p_logM1, p_gamma1, p_gamma2, p_sig0, p_b0, p_b1, p_b2, p_alpsat, p_eta, p_cfac, p_poff, p_roff, p_csbycdm, p_ap)

    
    print ("Starting iteration, ndim is ",ndim);
>>>>>>> 2cb08947c4825a1e0e38de69838ff8bfaff35728
    Nburn=5000
    pos = runchain(Nburn,sampler,"./sn_preet_gama_output_cosmo_y08_full_fits/burnfile.dat_bosch_2013_loglin_alpsat_csbycdm","./sn_preet_gama_output_cosmo_y08_full_fits/burnpredfile.dat_bosch_2013_loglin_alpsat_csbycdm",p_0);
    
    sampler.reset();
    
    Ntotal=10000
    posfinal = runchain(Ntotal,sampler,"./sn_preet_gama_output_cosmo_y08_full_fits/chainfile.dat_bosch_2013_loglin_alpsat_csbycdm","./sn_preet_gama_output_cosmo_y08_full_fits/predfile.dat_bosch_2013_loglin_alpsat_csbycdm", pos);
    pool.close()
