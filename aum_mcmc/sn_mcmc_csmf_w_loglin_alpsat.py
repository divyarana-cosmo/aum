...
import os
sys.path.append("/mnt/home/student/cdivya/github/aum-master/csmf_bosch_2013_loglin_alpsat/lib/python2.7/site-packages")
...
def lnprior(x):
    logM0, logM1, gamma1, gamma2, sig0, b0, b1, b2, alpsat, eta, cfac, poff, roff , ap = x
    if 8.0<=logM0<=11.5 and 9.5<=logM1<=15 and 1<=gamma1<=4 and 0.01<=gamma2<=2 and 0.05<=sig0<=0.5 and -2<=b0<=2 and -2<=b1<=2 and -2<=b2<=2 and -2<=alpsat<=-1 and -1<=eta<=0.0 and 0.5<=ap<=5  and 0<=poff<=1.0 and 0.0<=roff<=0.2 and cfac>0:
...
def lnprob(x, data, icov, rbin, rsftarr, logMaarr, logMbarr, logMstelarr):
    ...
    for ii in range(len(rsftarr)):
        ...
        aum      = aumdict['%d'%ii]
        if ii<=3:
            pred     = esd(x, rbin[:7], zred, logMa, logMb, logMstel, aum=aum) 
            x0 = (ii)*len(rbin[:7])
            x1 = x0 + len(rbin[:7])
            #x1 = (ii+1)*len(rbin[:7])
        else:
...
if __name__ == "__main__":
    ...
    rdat = np.loadtxt('./sn_preet_gama_rand_output_cosmo_zu15_full_revised/avg_dsig_uni_0.dat');
    #rdat = np.loadtxt('./sn_preet_gama_rand_output_cosmo_zu15_full_revised/avg_dsig_0.dat');
    #rbin = rdat[:7,0]
    rbin = rdat[:10,0]
 
    #then get the full covariance
    cov = np.loadtxt('./sn_preet_gama_output_cosmo_zu15_full_revised/cov_no_boost_7_rbins_for_first_4_bins.dat',dtype=float,unpack=True)
    #cov = np.loadtxt('./sn_preet_gama_output_cosmo_zu15_full_revised/cov_no_boost.dat',dtype=float,unpack=True)
    #cov = np.loadtxt('./sn_preet_gama_output_cosmo_zu15_full_revised/cov_no_boost_7_rbins.dat',dtype=float,unpack=True)
    #cov = np.loadtxt('./sn_preet_gama_output_cosmo_zu15_full_revised/cov_no_boost.dat',dtype=float,unpack=True)
    #removing last bin
    #cov  = cov[:-10,:-10] 
    data = data 
    ...
    #if 8.0<=logM0<=11.5 and 9.5<=logM1<=15 and 1<=gamma1<=4 and 0.01<=gamma2<=2 and 0.05<=sig0<=0.5 and -2<=b0<=2 and 0<=b1<=2 and -2<=b2<=2 and -2<=alpsat<=-1 and -1<=eta<0 and 0.5<=ap<=5  and 0<=poff<=1.0 and 0.0<=roff<=0.2 and cfac>0:
    ...
    p_gamma1   = np.random.uniform(1,4,nwalkers)
    ...
    p_b1       = np.random.uniform(-1,2,nwalkers)
    p_b2       = np.random.uniform(-1,2,nwalkers)
    p_alpsat   = np.random.uniform(-2,-1,nwalkers)
    p_eta      = 0.0*np.random.uniform(-1,0.0,nwalkers)
    ...
    #pos = runchain(Nburn,sampler,"./sn_preet_gama_output_cosmo_zu15_full_revised_fits/burnfile_bosch_2013_loglin_alpsat_uni_7_rbins_const_alpha.dat","./sn_preet_gama_output_cosmo_zu15_full_revised_fits/burnpredfile_bosch_2013_loglin_alpsat_uni_7_rbins_const_alpha.dat",p_0);
    #pos = runchain(Nburn,sampler,"./sn_preet_gama_output_cosmo_zu15_full_revised_fits/burnfile_bosch_2013_loglin_alpsat_uni_no_lastbin.dat","./sn_preet_gama_output_cosmo_zu15_full_revised_fits/burnpredfile_bosch_2013_loglin_alpsat_uni_no_lastbin.dat",p_0);
    sampler.reset();
    
    Ntotal=10000
    posfinal = runchain(Ntotal,sampler,"./sn_preet_gama_output_cosmo_zu15_full_revised_fits/chainfile_bosch_2013_loglin_alpsat_uni_7_rbins_for_first_4_bins_const_alpha.dat","./sn_preet_gama_output_cosmo_zu15_full_revised_fits/predfile_bosch_2013_loglin_alpsat_uni_7_rbins_for_first_4_bins_const_alpha.dat", pos);
    #posfinal = runchain(Ntotal,sampler,"./sn_preet_gama_output_cosmo_zu15_full_revised_fits/chainfile_bosch_2013_loglin_alpsat_uni_7_rbins_const_alpha.dat","./sn_preet_gama_output_cosmo_zu15_full_revised_fits/predfile_bosch_2013_loglin_alpsat_uni_7_rbins_const_alpha.dat", pos);
    pool.close()
